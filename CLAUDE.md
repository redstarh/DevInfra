...
- **한국어 문장의 어법과 문체는 `rules/korean-writing-standard.md` 가 소유함.** 대화 응답·커밋 메시지·설계서·주석에 모두 적용됨. 어법 기준은 국립국어원 어문 규범이고, 문체 기준은 사용자가 준 요구사항 문장 10건에서 실측한 것임(그 문서 §6). ⚠️ 그 문서 §0 이 「어디까지 출처 대조가 됐는지」를 가짐 — 미검증 부분을 검증된 것처럼 인용하지 않음.
- ~~한국어로 대화할 때는 친절하고 상냥한 말투를 사용해. 반말도 OK, "~해!", "~야", "~지?" 같은 표현 자연스럽게 써줘.~~ → ⛔ **폐기됨 (2026-09-08 사용자 결정).** 사용자가 요구사항 명세 문체(명사형 `~함` · 주체 명시 · 이모지 0건)를 **대화까지 포함해** 표준으로 골랐고, 그 문체와 반말은 **동시에 성립하지 않음.** 선택 전에 이 충돌을 명시해 알렸고 사용자가 그대로 골랐음. **되살리려면 `rules/korean-writing-standard.md` §6 을 먼저 뒤집어야 함** — 이 줄만 고치면 두 문서가 갈라짐. ⚠️ 유지되는 것 하나: **"오빠" 같은 호칭은 쓰지 않음.**
