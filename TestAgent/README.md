...
**검사 자체가 무력화되지 않았는지**는 mutation 러너로 확인한다. 규약 사본에 결함을 하나씩 주입해
validator가 실제로 잡는지 대조하며, 원본 트리는 수정하지 않는다.

```bash
./TestAgent/scripts/mutation_test.sh
```

문구 검사만 늘리면 "파일이 존재한다"까지만 보증하는 껍데기가 되기 쉽다. 검사를 추가할 때 이 러너에
대응 케이스를 함께 넣어, 그 검사가 없으면 실제로 red가 되는지 먼저 확인한다.
