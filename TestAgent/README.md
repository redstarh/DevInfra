...
대응 케이스를 함께 넣어, 그 검사가 없으면 실제로 red가 되는지 먼저 확인한다. 각 케이스는 종료 코드뿐
아니라 **기대 FAIL 메시지**까지 대조하므로, 의도한 검사가 잡았는지 다른 단정이 먼저 걸렸는지 구분된다.

**실행 환경 가정** — 두 러너는 `bash` 3.2 이상, POSIX `find`, `grep`(`-I` `-i` `-F` 지원), `sed`,
`mktemp -d`를 가정한다. `sed -i`는 BSD와 GNU의 문법이 달라 쓰지 않고 임시 파일 경유로 처리하므로
macOS와 Linux에서 같게 동작한다. 금지 문구 검사는 심볼릭 링크를 따라가지 않는다.
